...
对除了preif级以外：在下一拍将stage_valid信号拉低。并在当拍拉低stage_to_next_stage_valid信号，将该条指令标记为无效。

preif级：根据flush信号重新选取nextpc的值，并立即更新preif_pc。

if级：在当前if级指令正在等待icache返回数据的情况下置起if_inst_cancel信号，标记当前有一个回收到但没用的dataok信号。
