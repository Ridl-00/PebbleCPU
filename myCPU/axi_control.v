...
module axi_control(
    ...
    always @ (posedge clk) begin
        if (rst) begin
            ...
            araddr <= 32'b0;
            arlen <= 4'b0000;
            arsize <= 3'b010;//默认访存4字节
            arburst <= 2'b01;//所有读写均用burst，通过修改len调整读写长度
            arlock <= 2'b00;
            arcache <= 4'b0000;
            arprot <= 3'b000;
            arvalid <= 1'b0;
            ...
            case (1'b1)
                ...
                stage[3]:begin
                    if (arready) begin//arready后没有向cache提供驱动addrok的逻辑
                        arvalid <= 1'b0;
                        araddr <= 32'b0;
                        rready <= 1'b1;
                        icache_offset <= 4'd0;
                ...
                stage_w[0]:begin
                    if (stage[1]) begin
                        if (dcache_wen_buffer) begin
                            ...
                            awlen <= 4'b0;
                            case (uncache_wen_buffer)//目前cpu写字节或半字提供的data是重复该字节或半字（4或2次）
